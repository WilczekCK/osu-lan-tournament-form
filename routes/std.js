...
router.post('/add', function (req, res, next) {
  ...
  googleApi.getPlayers('GraczeSTD')
    .then(result => {
      ...
      osu.get_user({ u: req.body.id, m: "0" })
        .then(result => {
          if (utilities.prepareUser(result, req.body)) {
            googleApi.addPlayer(utilities.prepareUser(result, req.body), 'GraczeSTD')
              .then(result => {
